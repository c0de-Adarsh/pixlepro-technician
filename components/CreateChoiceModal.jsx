...
export default function CreateChoiceModal({
  ...
  const handleContinue = () => {
    onClose();
    if (selectedType === "time_off") {
      if (onContinueTimeOff) onContinueTimeOff();
    } else if (selectedType === "lead") {
      if (onContinueLead) onContinueLead();
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-white dark:bg-[#0E1E31] border border-slate-200/90 dark:border-slate-800 rounded-3xl shadow-2xl p-6 z-10 text-slate-800 dark:text-slate-100 space-y-5"
          >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2.5 pt-1">
                ...
                <label
                  onClick={() => setSelectedType("time_off")}
                  className={`flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all ${
                    selectedType === "time_off"
                      ? "border-[#D31010] bg-red-50/40 dark:bg-red-950/20 shadow-sm"
                      : "border-slate-200 dark:border-slate-800 hover:border-slate-300"
                  }`}
                >
                  <input
                    type="radio"
                    name="createChoice"
                    checked={selectedType === "time_off"}
                    onChange={() => setSelectedType("time_off")}
                    className="w-4 h-4 text-[#D31010] accent-[#D31010]"
                  />
                  <span className="text-xs font-extrabold text-slate-900 dark:text-white">
                    Time off
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-8 py-2.5 bg-[#D31010] hover:bg-[#b00d0d] text-white text-xs font-extrabold rounded-full shadow-md shadow-red-500/20 hover:shadow-lg transition-all cursor-pointer"
              >
                Continue
              </button>
            </div>
